...
export function PassPage({ profile, onBack }: PassPageProps) {
  ...
  const handleBuyPass = async (pass: Pass) => {
    ...
    if (profile.money < costMoney || profile.gems < costGems) {
      showAlert({ title: t("pass.insufficient_funds", "Fonds insuffisants"), type: "error" });
      return;
    }
    ...
    try {
      ...
      if (costMoney > 0)
        await logTransaction(
          ...
          "money",
          -costMoney,
          t("pass.purchase_transaction", "Achat Pass {name}").replace("{name}", tDb(pass.name)),
        );
      if (costGems > 0)
        await logTransaction(
          profile.uid,
      ...
      showAlert({ title: t("pass.activated_alert", "Pass activé !"), type: "success" });
    } catch (err) {
      console.error("Error buying pass", err);
      showAlert({ title: t("pass.purchase_error", "Erreur lors de l'achat"), type: "error" });
    }
  };
  ...
  const handleClaimReward = async (level: number, type: "free" | "premium") => {
    ...
    if (
      ...
      showAlert({
        title: t("pass.premium_required", "Pass Premium Requis"),
        subtitle: t("pass.premium_required_desc", "Vous devez activer le pass premium pour récupérer ces récompenses."),
        type: "error",
      });
      return;
    }
    ...
    try {
      ...
      if (reward.type === "money" && reward.amount)
        await logTransaction(
          ...
          "money",
          reward.amount,
          t("pass.reward_transaction", "Récompense Pass Niveau {level}").replace("{level}", level.toString()),
        );
      ...
      showReward({
        ...
        title: t("pass.reward_level_title", "Récompense Palier {level} !").replace("{level}", level.toString()),
        card:
          reward.type === "card" && reward.cardId
            ? { name: t("missions.card_unlocked", "Carte Débloquée"), imageUrl: "placeholder" }
      ...
      showAlert({ title: t("pass.claim_error", "Erreur lors de la récupération"), type: "error" });
    }
  };
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {displayPasses.length === 0 ? (
            <div className="flex items-center justify-center h-full text-gray-500 font-bold">
              {t("pass.no_pass", "Aucun Pass {status}.").replace("{status}", activeTab === "active" ? t("pass.status_active", "disponible") : t("pass.status_archive", "archivé"))}
            </div>
          ) : (
                    ...
                    <div className="flex-1 pr-4">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        >
                          {!pass.conditionType ||
                          pass.conditionType === "global"
                            ? t("pass.general_pass", "Pass Général")
                            : pass.conditionType === "league"
                              ? t("pass.competition_pass", "Pass Compétition")
                              : pass.conditionType === "country"
                                ? t("pass.country_pass", "Pass Pays")
                                : pass.conditionType === "team"
                                  ? t("pass.team_pass", "Pass Équipe")
                                  : pass.conditionType === "season"
                                    ? t("pass.season_pass", "Pass Saison")
                                    : t("pass.special_pass", "Pass Spécial")}
                    ...
                    <div className="flex flex-col items-end gap-3 shrink-0">
                      {isOwned ? (
                        <div className="px-5 py-2.5 bg-green-500 text-white shadow-[0_0_15px_rgba(34,197,94,0.4)] rounded-lg text-xs md:text-sm font-black uppercase tracking-widest flex items-center gap-2">
                          <Check className="w-4 h-4" /> {t("pass.active_status", "Actif")}
                        </div>
                      ) : (
                      ...
                      <div className="text-[11px] md:text-xs font-black tracking-widest text-purple-400 uppercase drop-shadow-md bg-black/50 border border-purple-500/30 px-3 py-1 rounded">
                        {t("pass.levels_count", "{count} Niveaux").replace("{count}", pass.levels.length.toString())}
                      </div>
                    </div>
                  </div>
                </Card>
  ...
  return (
    <div className="flex flex-col h-full bg-black">
      <div className="flex-1 overflow-y-auto p-4 pb-20">
        ...
        >
          <ChevronLeft className="w-4 h-4" /> {t("pass.back_to_passes", "Retour aux passes")}
        </button>
        ...
        <div className="mb-6">
          <div className="flex justify-between items-end mb-2">
            <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest">
              {t("pass.my_progress", "Ma progression")}
            </h3>
        ...
        >
          ...
          <div className="relative z-10 p-6 flex flex-col sm:flex-row items-center sm:items-end text-center sm:text-left justify-between w-full h-full mt-auto">
            ...
            {!isOwned ? (
              ...
              >
                <span className="text-sm">{t("pass.activate_premium", "Activer le Pass Premium")}</span>
              ...
              <div className="w-[85%] sm:w-[250px] py-3.5 bg-green-500 text-white shadow-[0_0_20px_rgba(34,197,94,0.4)] rounded-xl font-black uppercase flex items-center justify-center gap-2 shrink-0 text-sm">
                <Check className="w-5 h-5" /> {t("pass.premium_active", "Pass Premium Actif")}
              </div>
            )}
          </div>
        </Card>
                    ...
                    >
                      ...
                      {claimedFree && (
                        <div className="text-[10px] sm:text-xs text-green-500 font-black uppercase flex items-center gap-1 justify-center">
                          <Check className="w-3 h-3 sm:w-4 sm:h-4" /> {t("pass.reward_claimed", "Récupéré")}
                        </div>
                      )}
                    </Card>
                  ) : (
                  ...
                  {level.premiumReward ? (
                    ...
                    >
                      <div className="text-[10px] sm:text-xs font-black text-purple-400 uppercase mb-2 flex items-center justify-center gap-1 w-full bg-purple-500/10 rounded py-1 border border-purple-500/20">
                        <Lock className="w-3 h-3" /> {t("pass.premium", "Premium")}
                      </div>
                        ...
                        <>
                          ...
                          <div className="text-[10px] sm:text-xs font-black italic text-white mb-2 leading-tight h-8 sm:h-auto overflow-hidden text-ellipsis drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)]">
                            {skins[level.premiumReward.skinId]?.name ||
                              t("pass.skin_exclusive", "Skin Exclusif")}
                          </div>
                        </>
                      ) : (
                        <>
                      ...
                      {(!unlocked || !isOwned) && (
                        <div className="text-[10px] sm:text-xs text-gray-500 font-black uppercase flex items-center justify-center gap-1 bg-black/50 py-1 rounded w-full border border-white/5">
                          <Lock className="w-3 h-3 sm:w-4 sm:h-4" /> {t("pass.locked", "Bloqué")}
                        </div>
                      )}
                    </Card>
                  ) : (
