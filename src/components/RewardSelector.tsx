...
export const RewardSelector: React.FC<RewardSelectorProps> = ({
  ...
  const filteredTemplates =
    isFanzContext && currentFanzId && currentFanzId !== "global"
      ? fanzTemplates.filter(
          (t) => t.id === currentFanzId && t.isActive !== false,
        )
      : fanzTemplates.filter((t) => t.isActive !== false);

  const allSkins = filteredTemplates.flatMap((t) =>
    (t.skins || []).map((s) => ({ ...s, templateName: t.name })),
  );
  const allEmotes = filteredTemplates.flatMap((t) =>
    (t.emotes || []).map((e) => ({ ...e, templateName: t.name })),
  );
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex gap-2">
        ...
        {["money", "gems", "boost", "energy", "xp", "team_slot"].includes(
          reward?.type || "money",
        ) && (
          <input
            type="number"
      ...
      {reward?.type === "choice" && (
        <div className="mt-2 space-y-2 p-2 border border-gray-600 rounded bg-black/20">
          ...
          {(reward.choices || []).map((choice, cIdx) => (
            <div
              key={cIdx}
              className="flex gap-2 items-start border-l-2 border-orange-500 pl-2"
            >
      ...
      {reward?.type === "xp" && (
        <select
          ...
          onChange={(e) =>
            onChange({ ...reward, statName: e.target.value as any })
          }
