  ...
  return (
    <tr className={`hover:bg-white/5 transition-colors group ${isDirty ? 'bg-orange-500/5' : ''}`}>
      <td className="px-2 py-2 align-middle border-b border-white/5 min-w-[200px]">
        <label className="block text-[9px] font-bold uppercase tracking-widest text-gray-500 mb-1">Titre & ID</label>
        <input type="text" value={localMission.title || ''} onChange={e => handleChange('title', e.target.value)} className="w-full text-sm font-bold p-1 bg-black text-white rounded border border-white/10 mb-1" />
      ...
      <td className="px-2 py-2 align-middle border-b border-white/5 min-w-[250px]">
        ...
        <textarea value={localMission.description || ''} onChange={e => handleChange('description', e.target.value)} className="w-full text-xs p-1 bg-black text-white rounded border border-white/10 h-10" placeholder="Description..."></textarea>
      </td>
