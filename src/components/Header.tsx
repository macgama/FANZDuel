...
export function Header({
      ...
      try {
        ...
        fanzSnapshot.docs.forEach((doc) => {
          ...
          if (data.isActive !== false) {
            const fMax =
              data.ferveurConfig?.ranges?.[data.ferveurConfig.ranges.length - 1]
                ?.max || 150000;
            calculatedMax += fMax;
          }
        });
  ...
  useEffect(() => {
    ...
    const calculateTime = () => {
      ...
      setCountdownInfiniteEnergy(
        getTimeLeftString(profile.infiniteEnergyUntil),
      );
  ...
  const isXpBoostActive =
    profile.boostXpUntil && new Date(profile.boostXpUntil) > new Date();
  const isInfiniteEnergyActive =
    profile.infiniteEnergyUntil &&
    new Date(profile.infiniteEnergyUntil) > new Date();
  const isAntiMalusActive = (profile.antiMalusMatches || 0) > 0;
  const isDoubleGainsActive =
    profile.doubleGainsUntil && new Date(profile.doubleGainsUntil) > new Date();

  const currentFerveur = profile.ferveurPoints || 0;
  const ferveurProgressPercent = Math.min(
    100,
    Math.max(0, (currentFerveur / maxFerveur) * 100),
  );
        ...
        <div className={cn("w-full mx-auto flex items-start justify-between px-4 md:px-6 py-4", wide ? "max-w-none" : "max-w-[600px]")}>
          ...
          {variant === "subpage" ? (
            ...
            <div
              ...
              onClick={() =>
                onHomeClick ? onHomeClick() : setShowProfileModal(true)
              }
            >
              ...
              <div className="flex items-center gap-1 sm:gap-1.5 group/energy relative">
                <div className="relative">
                  <img
                    ...
                    alt="Energy"
                    className={cn(
                      "w-3.5 h-3.5 sm:w-4 sm:h-4 object-contain",
                      isInfiniteEnergyActive && "animate-pulse",
                    )}
                    referrerPolicy="no-referrer"
                  />
                ...
                <span
                  className={cn(
                    ...
                    isInfiniteEnergyActive && "text-yellow-400",
                  )}
                >
              ...
              >
                <div
                  className={cn(
                    ...
                    isXpBoostActive ? "bg-yellow-500" : "bg-orange-500",
                  )}
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-[8px] sm:text-[10px] font-black text-white uppercase tracking-tighter flex items-center gap-1">
                    ...
                    {isXpBoostActive && (
                      <Star className="w-2 h-2 text-yellow-300 fill-yellow-300 animate-pulse" />
                    )}
                  </span>
                </div>
              </div>
