...
import { doc, getDoc, updateDoc, setDoc, collection, getDocs, query, where, onSnapshot, arrayUnion, increment } from 'firebase/firestore';
...
import { Trophy, Lock, Unlock, Star, Info, ArrowLeft, Shield, Brain, Heart, Eye, MessageCircle, Users, Flame, Activity, Database, Clock, Trash2, FastForward, ChevronUp, CheckCircle, RefreshCw, Layers, Smile, ChevronLeft, ChevronRight, Check, Gift, X, Target, Plus, Minus, Zap } from 'lucide-react';
...
export function FanzDetails({ fanzId, userProfile, onBack }: FanzDetailsProps) {
  ...
  const [selectedMuseumCard, setSelectedMuseumCard] = useState<{card: DuelCard, isUnlocked: boolean, requirements: any[], canAfford: boolean} | null>(null);
  ...
  const ferveurPath = React.useMemo(() => {
    ...
    if (configToUse) {
      const maxPoints = configToUse.ranges?.[configToUse.ranges.length - 1]?.max || 50000;
      return generateFervorPath(maxPoints, configToUse);
    }
    if (template?.ferveurPath && template.ferveurPath.length > 0) return template.ferveurPath;
    if (fanz?.ferveurPath && fanz.ferveurPath.length > 0) return fanz.ferveurPath;
    return [];
  }, [template?.ferveurPath, fanz?.ferveurPath, fanzFervorConfig, template?.ferveurConfig]);
              ...
              <div className="flex flex-wrap items-center gap-1.5 mt-2 mb-1">
                ...
                {equippedSkinData.energyBonus && (
                  <div className="bg-blue-500/20 backdrop-blur-md border border-blue-500/30 rounded px-2 py-0.5 text-[9px] font-black uppercase text-blue-400 flex items-center gap-1 shadow-sm">
                    <Zap className="w-2.5 h-2.5" /> +{equippedSkinData.energyBonus} ENER Max
                  </div>
                )}
        ...
        <div className="min-h-[400px]">
              ...
              {userProfile.activeAction?.fanzId === fanz.id ? (
                  ...
                  <div className="relative w-full pb-6">
                    ...
                    >
                      <div className="flex flex-nowrap w-full">
                        {lifeActions
                          .filter(action => {
                            const isTemplateMatch = action.fanzTemplateId === template.id || !action.fanzTemplateId;
                            const isSkinMatch = !action.skinId || action.skinId === fanz.equippedSkin;
                ...
                <div className="grid grid-cols-2 gap-4">
                  {Object.entries(statLabels).map(([stat, label]) => {
                    ...
                    const bonusLevel = equippedSkinData?.statsBonus?.[stat as keyof NonNullable<typeof equippedSkinData>['statsBonus']] || 0;
                        ...
                        <div className="flex items-center gap-3 w-full justify-center">
                          ...
                          <div className="text-left">
                            ...
                            <div className="text-lg sm:text-xl font-black leading-tight flex items-center gap-1">
                              Niv. {totalLevel}
                              {bonusLevel > 0 && <span className="text-[10px] text-blue-400">+{bonusLevel}</span>}
                            </div>
                          </div>
                        </div>
            ...
            {activeTab === 'ferveur' && (
              <div className="flex flex-col gap-6 pb-20">
                ...
                <div className="px-2 relative cursor-pointer group" onClick={() => {
                  const nextStep = ferveurPath.find(l => (fanz.ferveurPoints || 0) < l.pointsRequired);
                  if (nextStep) {
                    const el = document.getElementById(`fanz-ferveur-node-${nextStep.pointsRequired}`);
                    if (el) el.scrollIntoView({ behavior: 'smooth', block: 'center' });
                  } else if (ferveurPath.length > 0) {
                    const el = document.getElementById(`fanz-ferveur-node-${ferveurPath[ferveurPath.length - 1].pointsRequired}`);
                    if (el) el.scrollIntoView({ behavior: 'smooth', block: 'center' });
                  }
                }}>
                  ...
                  <div className="flex justify-between items-end mb-2 relative z-10">
                    ...
                    <span className="text-sm font-black text-orange-400 group-hover:text-orange-300 transition-colors">{(fanz.ferveurPoints || 0).toLocaleString()} <span className="text-xs text-orange-500/50">/ {maxFerveurPoints.toLocaleString()} PTS</span></span>
                  </div>
                  <div className="h-4 w-full bg-gray-900/80 rounded-full overflow-hidden border border-gray-800 relative z-10">
                    <motion.div 
                      initial={{ width: 0 }}
                      animate={{ width: `${Math.min(100, Math.max(0, ((fanz.ferveurPoints || 0) / maxFerveurPoints) * 100))}%` }}
                              ...
                              <div className={`p-2 sm:p-4 rounded-xl sm:rounded-2xl border backdrop-blur-sm transition-all duration-500 bg-black/60 shadow-xl ${
                                ...
                                <div className="text-center">
                                  <div className={`text-sm sm:text-lg font-black italic uppercase tracking-tighter sm:mb-3 mb-1.5 drop-shadow-md ${isUnlocked && !isClaimed ? 'text-green-400' : 'text-gray-400'}`}>
                                    {['money', 'gems', 'boost', 'energy', 'xp'].includes(step.reward?.type || '') ? `+${step.reward?.amount} ${step.reward?.type === 'money' ? '$' : step.reward?.type}` : step.reward?.type === 'skin' ? 'Skin' : step.reward?.type === 'emote' ? 'Emote' : step.reward?.type === 'card' ? 'Carte' : step.reward?.type === 'action' ? 'Action' : step.reward?.type}
                                  </div>
                                        ...
                                        customReward.type === 'card' ? (
                                          customReward.cardId ? (
                                            <div className="w-16 h-16 rounded-[inherit] overflow-hidden p-1 absolute">
                                              <img src={allCards.find(c => c.id === customReward.cardId)?.imageUrl} className="w-full h-full object-cover rounded-[inherit] border border-white/20" />
                                            </div>
                                          ) : <Layers className="w-8 h-8" />
                                        ) : customReward.type === 'skin' ? (
                                          customReward.skinId ? (() => {
                                            const skin = allSkins.find(s => s.id === customReward.skinId);
                                            return skin ? (
                                              <div className="absolute inset-0 z-0 opacity-50">
                                                <OptimizedMedia type={skin.videoUrl ? 'video' : 'image'} src={skin.videoUrl || skin.imageUrl || null} poster={skin.imageUrl} className="w-full h-full object-cover" autoPlay loop muted />
                                              </div>
                                            ) : <Shield className="w-8 h-8" />;
                                          })() : <Shield className="w-8 h-8" />
                                        ) : customReward.type === 'emote' ? (
                                          customReward.emoteId ? (() => {
                                            const emote = allEmotes.find(e => e.id === customReward.emoteId);
                                            return emote ? (
                                              <div className="absolute inset-0 z-0 opacity-30 flex items-center justify-center pointer-events-none p-2">
                                                <OptimizedMedia type={emote.videoUrl ? 'video' : 'image'} src={emote.videoUrl || emote.imageUrl || null} poster={emote.imageUrl} className="w-full h-full object-contain" autoPlay loop muted />
                                              </div>
                    ...
                    .map(({ card, isUnlocked, requirements, canAfford }) => {
                      ...
                      return (
                        <motion.div
                          ...
                          onClick={() => setSelectedMuseumCard({card, isUnlocked, requirements, canAfford})}
            ...
            {activeTab === 'skins' && (
              <div className="grid grid-cols-2 gap-3">
                ...
                {(template.skins || [])
                  ...
                  const rarityBorder = 
                    skin.rarity === 'legendary' ? 'border-[3px] border-yellow-500 shadow-[0_0_15px_rgba(234,179,8,0.5)]' :
                    skin.rarity === 'epic' ? 'border-2 border-purple-500 shadow-[0_0_10px_rgba(168,85,247,0.3)]' :
                    skin.rarity === 'rare' ? 'border-2 border-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.3)]' :
                    'border border-gray-600';

                  const bonusClass = "h-5 sm:h-6 min-w-[70px] sm:min-w-[80px] justify-center backdrop-blur-md shadow-lg border text-white text-[9px] sm:text-[10px] font-black uppercase px-2 py-0.5 flex items-center shadow-black/50 drop-shadow-md rounded-md shrink-0";
                  ...
                  return (
                    ...
                    >
                      <div className="absolute top-2 left-2 z-20 flex flex-col gap-1 items-start">
                        ...
                        {skin.rarity && (
                          <div className={`px-2 py-1 rounded text-[9px] sm:text-[10px] font-black uppercase shadow-lg drop-shadow-md tracking-wider ${
                            skin.rarity === 'legendary' ? 'bg-gradient-to-r from-yellow-600 to-yellow-400 text-black border border-yellow-200' :
                            skin.rarity === 'epic' ? 'bg-gradient-to-r from-purple-700 to-purple-500 text-white border border-purple-300' :
                            skin.rarity === 'rare' ? 'bg-gradient-to-r from-blue-700 to-blue-500 text-white border border-blue-300' :
                            'bg-gradient-to-r from-gray-600 to-gray-400 text-white border border-gray-300'
                          }`}>
                            {skin.rarity === 'legendary' ? 'Légendaire' : skin.rarity === 'epic' ? 'Épique' : skin.rarity === 'rare' ? 'Rare' : 'Commun'}
                          </div>
                        )}
                      </div>
                      {!isUnlocked && (
              ...
              <div className="flex flex-col gap-6 my-auto">
                ...
                >
                  {selectedMuseumCard.card.imageUrl ? (
                    ...
                    <div className={`w-full h-full rounded-2xl flex flex-col items-center justify-center bg-gradient-to-br ${cardTypeStyles[selectedMuseumCard.card.type]?.bg || 'from-gray-800 to-gray-900'} border-4 ${cardTypeStyles[selectedMuseumCard.card.type]?.border || 'border-gray-700'}`}>
                      <Zap className="w-24 h-24 mb-6 opacity-50" />
                      <h3 className="text-3xl font-black italic uppercase text-center px-4">{selectedMuseumCard.card.name}</h3>
                    </div>
                  )}
                ...
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className={`${cardTypeStyles[selectedMuseumCard.card.type]?.text === 'text-green-500' ? 'bg-green-500' : cardTypeStyles[selectedMuseumCard.card.type]?.text === 'text-red-500' ? 'bg-red-500' : 'bg-blue-500'} text-white text-xs font-black px-2 py-1 rounded-full uppercase`}>
                    {cardTypeStyles[selectedMuseumCard.card.type]?.label || selectedMuseumCard.card.type}
                  </div>
                </div>

                <div className="bg-black/50 rounded-xl p-4 mb-6 border border-white/5">
                  <p className="text-sm text-gray-300 mb-4">{selectedMuseumCard.card.description}</p>
                  
                  {selectedMuseumCard.card.effects && selectedMuseumCard.card.effects.length > 0 && (
                    <div className="space-y-1 mb-2">
                       <span className="text-xs font-bold text-gray-400 uppercase">Effets:</span>
                       {selectedMuseumCard.card.effects.map((effect, i) => (
                         <div key={i} className={`text-sm font-black ${cardTypeStyles[selectedMuseumCard.card.type]?.text || 'text-white'} bg-white/5 px-2 py-1 rounded uppercase tracking-wider`}>
                           {effectLabels[effect.type] || effect.type} {effect.value && effect.value > 0 ? '+' : ''}{effect.value}
                ...
                {!selectedMuseumCard.isUnlocked ? (
                        ...
                        <ul className="text-sm text-gray-300 space-y-1">
                          {selectedMuseumCard.requirements.map((req, i) => (
                            <li key={i} className="flex items-center gap-2">
                              ...
                              {req.type === 'skill' && req.skillName && `Compétence ${req.skillName} niv.${req.minLevel}`}
                              {req.type === 'ferveur' && `Ferveur minimale: niv.${req.minLevel}`}
                              {req.type === 'rank' && `Rang minimal: ${req.minLevel}`}
                            </li>
                          ))}
                        </ul>
                      </div>
                    ...
                    {selectedMuseumCard.card.price ? (
                      <Button 
                        onClick={() => {
                          ...
                          setPurchaseConfirm({ type: 'card', item: selectedMuseumCard.card });
                        }}
                      ...
                      >
                        {selectedMuseumCard.canAfford ? (
                          <>Débloquer pour {selectedMuseumCard.card.price.money} <img src={LOGOS.money} alt="Money" className="w-4 h-4 ml-1 inline-block" /></>
                        ) : 'Fonds insuffisants'}
                      </Button>
                    ) : (
                      selectedMuseumCard.requirements.length > 0 && (
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    {fanz.equippedCards?.includes(selectedMuseumCard.card.id) ? (
              ...
              <div className="text-center mb-6">
                ...
                {purchaseConfirm.type === 'skin' && (
                  <div className="mt-4 flex flex-wrap justify-center gap-2">
                    {/* Advantages */}
                    {purchaseConfirm.item.statsBonus && Object.entries(purchaseConfirm.item.statsBonus).map(([stat, val]) => {
                      ...
                      const statMap: Record<string, string> = {
                        force: 'FOR', mental: 'MEN', intelligence: 'INT', creativity: 'CRE',
                        bluff: 'BLU', social: 'SOC', charisma: 'CHA', endurance: 'END'
                      };
                      return (
                        <span key={stat} className="bg-blue-600/90 shadow-lg border border-blue-400/40 text-white text-[11px] font-black uppercase px-2 py-1 rounded flex items-center">+{Number(val)} {statMap[stat]}</span>
                      );
                    })}
                    {purchaseConfirm.item.energyBonus ? <span className="bg-amber-500/90 shadow-lg border border-amber-400/40 text-white text-[11px] font-black uppercase px-2 py-1 rounded flex items-center gap-1"><img src={LOGOS.energy} alt="" className="w-3 h-3 drop-shadow-md" />+{purchaseConfirm.item.energyBonus} Max</span> : null}
                    {purchaseConfirm.item.fervorBonus ? <span className="bg-purple-600/90 shadow-lg border border-purple-400/40 text-white text-[11px] font-black uppercase px-2 py-1 rounded">+{purchaseConfirm.item.fervorBonus}% Ferv</span> : null}
                    {purchaseConfirm.item.moneyBonus ? <span className="bg-yellow-500/90 shadow-lg border border-yellow-300/40 text-white text-[11px] font-black uppercase px-2 py-1 rounded flex items-center gap-1"><img src={LOGOS.money} alt="" className="w-3 h-3 drop-shadow-md" />+{purchaseConfirm.item.moneyBonus}%</span> : null}
                    {purchaseConfirm.item.gemsBonus ? <span className="bg-pink-500/90 shadow-lg border border-pink-300/40 text-white text-[11px] font-black uppercase px-2 py-1 rounded flex items-center gap-1"><img src={LOGOS.gems} alt="" className="w-3 h-3 drop-shadow-md" />+{purchaseConfirm.item.gemsBonus}%</span> : null}
                    {purchaseConfirm.item.boostBonus ? <span className="bg-orange-500/90 shadow-lg border border-orange-400/40 text-white text-[11px] font-black uppercase px-2 py-1 rounded flex items-center gap-1"><img src={LOGOS.boost} alt="" className="w-3 h-3 drop-shadow-md" />+{purchaseConfirm.item.boostBonus}%</span> : null}
                    
                    {/* Reductions */}
                    {purchaseConfirm.item.energyCostReduction ? <span className="bg-blue-800/90 shadow-lg border border-gray-400/40 text-white text-[11px] font-black uppercase px-2 py-1 rounded flex items-center gap-1">-{purchaseConfirm.item.energyCostReduction}% Cout / <img src={LOGOS.energy} alt="" className="w-3 h-3 drop-shadow-md" /></span> : null}
                    {purchaseConfirm.item.moneyCostReduction ? <span className="bg-yellow-800/90 shadow-lg border border-gray-400/40 text-white text-[11px] font-black uppercase px-2 py-1 rounded flex items-center gap-1">-{purchaseConfirm.item.moneyCostReduction}% Cout / <img src={LOGOS.money} alt="" className="w-3 h-3 drop-shadow-md" /></span> : null}
                    {purchaseConfirm.item.gemsCostReduction ? <span className="bg-pink-800/90 shadow-lg border border-gray-400/40 text-white text-[11px] font-black uppercase px-2 py-1 rounded flex items-center gap-1">-{purchaseConfirm.item.gemsCostReduction}% Cout / <img src={LOGOS.gems} alt="" className="w-3 h-3 drop-shadow-md" /></span> : null}
                    {purchaseConfirm.item.boostCostReduction ? <span className="bg-orange-800/90 shadow-lg border border-gray-400/40 text-white text-[11px] font-black uppercase px-2 py-1 rounded flex items-center gap-1">-{purchaseConfirm.item.boostCostReduction}% Cout / <img src={LOGOS.boost} alt="" className="w-3 h-3 drop-shadow-md" /></span> : null}
                    
                    {/* Specials */}
                    {purchaseConfirm.item.specialCardId ? <span className="bg-indigo-500/90 shadow-lg border border-indigo-400/40 text-white text-[11px] font-black uppercase px-2 py-1 rounded">1 Carte Spé</span> : null}
                    {purchaseConfirm.item.specialActionId ? <span className="bg-emerald-500/90 shadow-lg border border-emerald-400/40 text-white text-[11px] font-black uppercase px-2 py-1 rounded">1 Action Spé</span> : null}
                  </div>
                )}
              </div>
                    ...
                    <>
                      ...
                      {(rewardModal.rewardType === 'choice' || rewardModal.rewardType === 'card') && (
                        allCards.filter(c => {
                          ...
                          const isSkinMatch = !c.skinId || c.skinId === fanz.equippedSkin;
                      ...
                      {(rewardModal.rewardType === 'choice' || rewardModal.rewardType === 'action') && (
                        ...
                        .reduce((acc, action) => {
                          const existingIdx = acc.findIndex(a => a.name === action.name);
                          if (existingIdx !== -1) {
                            if (action.skinId && !acc[existingIdx].skinId) acc[existingIdx] = action;
